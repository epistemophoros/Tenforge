package com.github.b4ndithelps.tenforge.data;
...
import com.github.b4ndithelps.tenforge.network.SyncEnergyPacket;
...
public class SkillTransferManager {
    ...
    public OperationResult transferSkill(Player source, Player target, ResourceLocation skillId) {
        if (source.equals(target)) {
            return OperationResult.failure("tenforge.transfer.same_player");
        }
        ...
        if (sourceStorage == null || targetStorage == null) {
            return OperationResult.failure("tenforge.transfer.storage_error");
        }

        // Find the skill instance
        ManasSkillInstance skillInstance = findSkillInstance(sourceStorage, skillId);
        if (skillInstance == null) {
            return OperationResult.failure("tenforge.transfer.skill_not_found");
        }
        ...
        if (SkillRegistry.get().playerHasSkill(target, skillId)) {
            return OperationResult.failure("tenforge.transfer.target_has_skill");
        }
        ...
        return OperationResult.success("tenforge.transfer.success", 
    ...
    public OperationResult breakdownSkill(ServerPlayer player, ResourceLocation skillId, boolean fullBreakdown) {
        ...
        if (storage == null) {
            return OperationResult.failure("tenforge.breakdown.storage_error");
        }

        ManasSkillInstance skillInstance = findSkillInstance(storage, skillId);
        if (skillInstance == null) {
            return OperationResult.failure("tenforge.breakdown.skill_not_found");
        }

        ManasSkill skill = skillInstance.getSkill();
        
        // Calculate energy yield
        double energyYield = com.github.b4ndithelps.tenforge.util.SkillEnergyCalculator
        ...
        if (fullBreakdown) {
            return OperationResult.success("tenforge.breakdown.full_success",
                    skill.getName().getString(), String.format("%.0f", added[0]));
        } else {
            return OperationResult.success("tenforge.breakdown.extract_success",
                    skill.getName().getString(), String.format("%.0f", added[0]));
        }
    }
    ...
    public OperationResult forgeSkill(ServerPlayer player, ResourceLocation skillId) {
        ...
        if (dataOpt.isEmpty()) {
            return OperationResult.failure("tenforge.forge.unknown_skill");
        }
        ...
        if (SkillRegistry.get().playerHasSkill(player, skillId)) {
            return OperationResult.failure("tenforge.forge.already_owned");
        }
        ...
        SkillEnergyCapability.get(player).ifPresent(cap -> {
            if (cap.getEnergy() < data.creationCost) {
                result[0] = OperationResult.failure("tenforge.forge.insufficient_energy",
            ...
            if (epStorage == null || epStorage.getEp() < data.epCost) {
                result[0] = OperationResult.failure("tenforge.forge.insufficient_ep",
            ...
            result[0] = OperationResult.success("tenforge.forge.success", data.name);
        });

        if (result[0] == null) {
            result[0] = OperationResult.failure("tenforge.forge.unknown_error");
        }
    ...
    public double calculateTotalSkillValue(Player player) {
        ...
        double total = 0;
        for (ManasSkillInstance instance : storage.getLearnedSkills()) {
            total += com.github.b4ndithelps.tenforge.util.SkillEnergyCalculator
